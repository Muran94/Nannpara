  ...
  context 'ログインユーザー' do
    ...
    context '正常系' do
      ...
      it '正常に投稿が完了し、適切なflashメッセージが表示された状態でブログ記事詳細ページにリダイレクトすること' do
        ...
        find('#create-article-button').click # 右下のブログ記事作成ボタンをクリック
        sleep 0.3
        ...
        expect(current_path).to eq blog_article_path(Blog::Article.first)
    ...
    context '異常系' do
      it 'クライアントバリデーションに引っかかり、ブログ記事の投稿ができないこと' do
        ...
        # 文字数制限に引っかかった場合
        fill_in 'blog_article[title]', with: 'あ' * (Blog::Article::MAXIMUM_TITLE_LENGTH + 1)
        fill_in 'blog_article[content]', with: 'あ' * (Blog::Article::MAXIMUM_CONTENT_LENGTH + 1)
        click_button '作成'
