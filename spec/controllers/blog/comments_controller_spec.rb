    ...
    context "ログイン済み" do
      ...
      context "正常系" do
        it "コメントの投稿に成功し、適切なflash[:success]メッセージとともにブログ記事詳細ページにリダイレクトされること" do
          aggregate_failures do
            expect {post :create, params}.to change(Blog::Comment, :count).by(1)
            expect(Blog::Comment.first.user_id).not_to be_nil
      ...
      context "異常系" do
        ...
        it "コメントの投稿に失敗し、適切なflash[:error]メッセージとともにブログ記事詳細ページにリダイレクトされること" do
          aggregate_failures do
            expect {post :create, params}.not_to change(Blog::Comment, :count)
    ...
    context "ログイン済み" do
      context "コメントのオーナーである" do
        ...
        it "コメントの削除に成功し、適切なフラッシュメッセージとともにブログ記事詳細ページにリダイレクトされる。" do
          aggregate_failures do
            expect {delete :destroy, params}.to change(Blog::Comment, :count).by(-1)
      ...
      context "コメントオーナーでも、ブログ記事のオーナーでもない" do
        ...
        it "不正な操作なので、適切なメッセージとともにブログ記事詳細ページにリダイレクトさせる" do
          aggregate_failures do
            expect {delete :destroy, params}.not_to change(Blog::Comment, :count)
