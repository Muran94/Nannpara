...
RSpec.describe Blog::ArticlesController, type: :controller do
  ...
  describe 'GET #show' do
    ...
    context 'インスタンス変数' do
      it '@blog_articleに正しい値がセットされている' do
        ...
        expect(assigns(:new_blog_comment)).to be_a_new(Blog::Comment)
      end
    end
  end
  ...
  describe 'GET #new' do
    context 'ログイン済み' do
      ...
      context 'インスタンス変数' do
        it '@blog_articleに正しいユーザーIDを持つBlog::Articleの新インスタンスが代入されている' do
          aggregate_failures do
            expect(assigns(:blog_article)).to be_a_new Blog::Article
  ...
  describe 'POST #create' do
    ...
    context 'ログイン済み' do
      ...
      context '異常系' do
        let(:title) { 'あ' * (Blog::Article::MAXIMUM_TITLE_LENGTH + 1) }

        it 'バリデーションに引っかかった場合は、ブログ記事が作成されず、正しいflash[:error]メッセージとともにnewテンプレートがrenderされる' do
          aggregate_failures do
            expect { post :create, params: params }.not_to change(Blog::Article, :count)
  ...
  describe 'PATCH/PUT #update' do
    ...
    context 'ログイン済み' do
      context 'ブログ記事のオーナーである' do
        ...
        context '異常系' do
          let(:blog_article_after_title) { 'あ' * (Blog::Article::MAXIMUM_CONTENT_LENGTH + 1) }
    ...
    context 'ログイン済み' do
      context 'ブログ記事のオーナーである' do
        ...
        it 'ブログ記事の削除に成功し、正しいflash[:success]メッセージとともにブログ記事一覧ページにリダイレクトされること' do
          aggregate_failures do
            expect { delete :destroy, params: params }.to change(Blog::Article, :count).by(-1)
