...
RSpec.describe BlogArticle, type: :model do
  context 'バリデーションテスト' do
    context 'title' do
      ...
      context 'length' do
        it "#{BlogArticle::MAXIMUM_TITLE_LENGTH}文字以下ならバリデーションに引っかからず、超えていたらバリデーションに引っかかる" do
          aggregate_failures do
            expect(build_stubbed(:blog_article, title: 'a' * (BlogArticle::MAXIMUM_TITLE_LENGTH - 1)).valid?).to be_truthy
            expect(build_stubbed(:blog_article, title: 'a' * BlogArticle::MAXIMUM_TITLE_LENGTH).valid?).to be_truthy
            expect(build_stubbed(:blog_article, title: 'a' * (BlogArticle::MAXIMUM_TITLE_LENGTH + 1)).valid?).to be_falsy
          end
        end
      end
    end
    ...
    context 'content' do
      ...
      context 'length' do
        it "#{BlogArticle::MAXIMUM_CONTENT_LENGTH}文字以下ならバリデーションに引っかからず、超えていたらバリデーションに引っかかる" do
          aggregate_failures do
            expect(build_stubbed(:blog_article, content: 'a' * (BlogArticle::MAXIMUM_CONTENT_LENGTH - 1)).valid?).to be_truthy
            expect(build_stubbed(:blog_article, content: 'a' * BlogArticle::MAXIMUM_CONTENT_LENGTH).valid?).to be_truthy
            expect(build_stubbed(:blog_article, content: 'a' * (BlogArticle::MAXIMUM_CONTENT_LENGTH + 1)).valid?).to be_falsy
          end
        end
      end
    end
